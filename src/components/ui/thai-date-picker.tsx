...
export function ThaiDatePicker({
  ...
  useEffect(() => {
    ...
    // Initialize flatpickr with user's exact specification
    const instance = flatpickr(inputRef.current, {
      ...
      altInputClass: `w-full h-11 py-2.5 pl-10 pr-3.5 rounded-xl bg-[var(--bg-canvas)] text-sm font-medium text-[var(--fg-primary)] border outline-none focus:outline-none transition-all cursor-pointer select-none shadow-2xs ${
        error
          ...
          : "border-[var(--border-subtle)] hover:border-slate-400 dark:hover:border-slate-600 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20"
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      ...
      <div className="relative w-full group">
        <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-emerald-500 pointer-events-none z-10 transition-colors">
